...
from new_env.util.env_util import get_stat_in_paths, \
    create_stats_ordered_dict, get_asset_full_path
from new_env.base import SawyerXYZEnv

from new_env.util.rotation import euler2quat
from new_env.base import OBS_TYPE
...
class SawyerHammerEnv(SawyerXYZEnv):
    ...
    def step(self, action):
        ...
        reward, reachRew, reachDist, pickRew, hammerRew, hammerDist, screwDist = self.compute_reward(action, obs_dict,
                                                                                                     mode=self.rewMode)
        ...
        info = {'reachDist': reachDist, 'pickRew': pickRew, 'epRew': reward, 'goalDist': screwDist,
    ...
    def reset_model(self):
        ...
        self._state_goal = self.sim.model.site_pos[self.model.site_name2id('goal')] + self.sim.model.body_pos[
        ...
        self.obj_init_pos = self.sim.model.site_pos[self.model.site_name2id('screwHead')] + self.sim.model.body_pos[
            self.model.body_name2id('screw')]
        # self._set_obj_xyz_quat(self.obj_init_pos, self.obj_init_angle)
        self.maxHammerDist = np.linalg.norm(
            np.array([self.hammer_init_pos[0], self.hammer_init_pos[1], self.heightTarget]) - np.array(
                self.obj_init_pos)) + \
    ...
    def compute_reward(self, actions, obs, mode='orig'):
        ...
        print("under action compute reward")
        print(actions)
        print(hammerPos)
        print(objPos)
        print(self._state_goal[1])
        print(rightFinger, leftFinger)
        ...
        def hammerReward():
            ...
            if cond:
                hammerRew = 1000 * (self.maxHammerDist - hammerDist - screwDist) + c1 * (
                            np.exp(-((hammerDist + screwDist) ** 2) / c2) + np.exp(
                        -((hammerDist + screwDist) ** 2) / c3))
